...
let Segments = [];
const tableCells = new Array(24).fill("false");
let camData = {
	"JPO-001" : [],
	"JPO-002" : [],
	"JPO-003" : []
...
let firstElementJPO001 = camDataName["JPO-001"][0];
console.log("TEESSSS",firstElementJPO001, camDataName["JPO-001"])

function fillTableName(){
	let i = 1

	for(i = 1; i <= 24; i++){
		const liveName0Element = document.getElementById("liveName" + i);
		if (i <= 8){
			liveName0Element.textContent=camDataName["JPO-001"][i-1];
		}else if (i <= 16){
...
function inputData(floor, Name ,ID) {
	try {
		val = camData[floor].includes(ID)
		if (!val){
			camData[floor].push(ID)
			camDataName[floor].push(Name)
			console.log(camDataName[floor])
		}
	} catch (error) {
...
function livePerJPO(floor, config){
	// Object.keys(camData).forEach((floor) => {
	// 	camData[floor].forEach(element => {
	// 		usedCell = findUsedByIDCell(element)

	// 		if (usedCell != null) {
	// 			closeLive(element, usedCell)
	// 		}
	// 	})
	// })
	// let iName = 0;
...
function StartLiveSingle(ID, Name, Codec) {
	let buffer;
    let socket;
	...
	VE5.id = ID; // play the camera with given ID
	console.log(VE5.id)
	VE5.width = 1450;
	VE5.height = 820;
	...
	const videoCell = document.getElementById('liveContainer');
